...
export function useAppLabels({
  ...
  const loadingTitle = pickLang(uiLanguage, {
    ko: "Next AI Crew 로딩 중...",
    en: "Loading Next AI Crew...",
    ja: "Next AI Crewを読み込み中...",
    zh: "Next AI Crew 加载中...",
  });
  ...
  const effectiveUpdateStatus = forceUpdateBanner
    ? {
        ...
        release_url: updateStatus?.release_url ?? "https://github.com/Kevinjeenext/next-ai-crew/releases/latest",
        checked_at: Date.now(),
        enabled: true,
        repo: updateStatus?.repo ?? "Kevinjeenext/next-ai-crew",
        error: null,
      }
    : updateStatus;
  ...
  const updateReleaseUrl =
    ...
    `https://github.com/${effectiveUpdateStatus?.repo ?? "Kevinjeenext/next-ai-crew"}/releases/latest`;
