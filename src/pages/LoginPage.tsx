 ...
 * Graceful fallback: if OAuth providers aren't enabled in Supabase,
 * buttons show error and fade out, leaving email-only auth.
 *
  ...
  const handleOAuth = useCallback(async (provider: Provider) => {
    ...
    if (error) {
      ...
      if (msg.includes("provider") && (msg.includes("not enabled") || msg.includes("not supported") || msg.includes("unsupported"))) {
        // Provider not configured in Supabase — disable button
        setDisabledProviders((prev) => new Set(prev).add(provider));
        setMessage({
          text: `${provider.charAt(0).toUpperCase() + provider.slice(1)} 로그인이 아직 설정되지 않았습니다. 이메일로 로그인해주세요.`,
          type: "error",
        });
      } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-indigo-900">
      <div className="w-full max-w-md p-8 space-y-6">
        ...
        {/* OAuth Providers — hide section if all disabled */}
        ...
        {/* Divider — show "or" only if some OAuth still visible */}
        {!allProvidersDisabled && (
        ...
        {/* Email-only banner when all OAuth disabled */}
        {allProvidersDisabled && (
          <div className="rounded-lg p-3 text-center text-sm" style={{ backgroundColor: "rgba(99,102,241,0.15)", color: "#A5B4FC" }}>
            이메일로 로그인해주세요
          </div>
        )}
