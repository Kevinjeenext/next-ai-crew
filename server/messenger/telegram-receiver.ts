    ...
    try {
      ...
      if (isTelegramConflictError(status.lastError)) {
        ...
        if (!conflictWarnedInPause) {
          console.warn(`[Next AI Crew] telegram receiver paused: ${status.lastError}`);
          console.warn(
            `[Next AI Crew] telegram receiver hint: another bot instance is polling this token. retry in ${Math.round(
        ...
        conflictWarnedInPause = false;
        console.warn(`[Next AI Crew] telegram receiver error: ${status.lastError}`);
      }
    } finally {
      busy = false;
